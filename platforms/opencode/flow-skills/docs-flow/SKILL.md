---
name: docs-flow
description: Use when the developer says "update docs", "document X", "docs for X", "audit documentation", "fix the docs", or "knowledge base maintenance". Audits and updates the project knowledge base under flow-storage/{project,team,tasks}/, including ARCHITECTURE.md, CONVENTIONS.md, PATTERNS.md, DECISIONS.md, lessons-learned.md, onboarding.md. Living documents only — never creates archived/versioned copies (Rule 2). The full flow is in .ai-workflow/flows/docs-flow.md.
license: MIT
metadata:
  audience: developers
...
4. If a task just merged, propagates lessons-learned and patterns into project-level docs
...
The flow's last phase is a CRITICAL-gate commit (`docs: update {scope} — {summary}`). Without this, doc edits sit uncommitted and silently revert on the next branch switch. Opt-out via `--no-commit` if bundling with code changes that come later.
