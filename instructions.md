...
1. **Per-flow skill** at `.claude/skills/orchestrate-flow/SKILL.md` — gives the agent a strong description match on "orchestrate X" / "build everything for X" requests, with Claude-specific Phase 2 guidance (skip `git worktree add`, dispatch via Agent tool with `subagent_type: orchestrate-implementer`).
